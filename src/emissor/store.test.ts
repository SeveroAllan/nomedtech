...
test("salvarPrestador e obterPrestador persistem e recuperam configuracao por tenant", () => {
  initDb(":memory:");
  const prestador = {
    id: "medico-10",
    cnpj: "11222333000181",
    ...
    certPath: "/certs/medico10.p12",
    certPassword: "senha",
    ambiente: 1 as const,
    outputDir: "/output/medico-10",
    regTrib: { opSimpNac: 3 as const, regApTribSN: 1 as const, regEspTrib: 0 as const },
    pTotTribSN: 15.5,
  };

  salvarPrestador(prestador);
  const obtido = obterPrestador("medico-10");
  assert.ok(obtido);
  assert.equal(obtido.id, "medico-10");
  assert.equal(obtido.cnpj, "11222333000181");
  assert.equal(obtido.ambiente, 1);
  assert.equal(obtido.pTotTribSN, 15.5);
});

test("listVendas e proximoNDPS isolam dados por prestador_id", () => {
  initDb(":memory:");
  upsertVenda({ ...vendaBase("venda-medico-a"), prestador_id: "medico-a" });
  upsertVenda({ ...vendaBase("venda-medico-b"), prestador_id: "medico-b" });

  const vendasA = listVendas({ prestador_id: "medico-a" });
  assert.equal(vendasA.length, 1);
  assert.equal(vendasA[0].id_venda, "venda-medico-a");

  const vendasB = listVendas({ prestador_id: "medico-b" });
  assert.equal(vendasB.length, 1);
  assert.equal(vendasB[0].id_venda, "venda-medico-b");

  // Notas e proximoNDPS independentes
  registrarNota({ chave_acesso: "CHAVE-A-1", ndps: 10, prestador_id: "medico-a" });
  registrarNota({ chave_acesso: "CHAVE-B-1", ndps: 100, prestador_id: "medico-b" });

  assert.equal(proximoNDPS("medico-a"), 11);
  assert.equal(proximoNDPS("medico-b"), 101);
  assert.equal(proximoNDPS("medico-c"), 1); // Sem notas ainda
});
