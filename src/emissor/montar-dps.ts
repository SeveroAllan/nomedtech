...
export function momentoSP(agora: Date = new Date()): { dhEmi: string; dCompet: string } {
...
export function montarDps(
  ...
  const isMei = cfg.regTrib.opSimpNac === 2;
  ...
  return {
    infDps: {
      ...
      prest: {
        CNPJ: cfg.cnpj,
        ...(cfg.im && cfg.im !== '000000' && cfg.im !== '0' && cfg.im.trim().length >= 3 ? { IM: cfg.im.trim() } : {}),
      ...
      valores: {
        ...
        trib: {
          ...
          // E0676: Não é permitido o preenchimento de tributos federais para MEI
          ...(!isMei ? { tribFed: { piscofins: { CST: '08' } } } : {}),
          ...
          // E0710 proibe pTotTribSN para MEI, que deve usar indTotTrib: 0 (Decreto 8.264/2014)
          totTrib: isMei
            ? { indTotTrib: 0 }
            : { pTotTribSN: input.pTotTribSN ?? cfg.pTotTribSN },
        },
      },
      IBSCBS: {
        finNFSe: '0',
