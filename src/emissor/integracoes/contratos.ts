...
function cmdCriar(args: string[]): void {
  ...
  const prestadorId = flag(args, "--prestador") ?? "default";
  ...
  console.log(`Contrato criado com id=${id} [Prestador: ${prestadorId}]`);
...
function cmdListar(args: string[]): void {
  ...
  const contratos = listContratos(prestadorId, !todos);

  if (contratos.length === 0) {
    console.log(todos ? `Nenhum contrato cadastrado para prestador "${prestadorId}".` : `Nenhum contrato ativo para prestador "${prestadorId}".`);
    return;
  }

  console.log(`\nContratos [Prestador: ${prestadorId}]:`);
  console.log(`${"ID".padEnd(5)} ${"DOC".padEnd(18)} ${"NOME".padEnd(30)} ${"VALOR".padStart(10)} ${"DIA".padStart(4)} ${"ATIVO"}`);
...
function cmdDesativar(args: string[]): void {
  ...
  const prestadorId = flag(args, "--prestador");
  if (!idStr) {
    console.error("Uso: contratos.ts desativar <id> [--prestador <id>]");
    process.exit(1);
  }
  ...
  initDb();
  desativarContrato(id, prestadorId);
  console.log(`Contrato id=${id} desativado${prestadorId ? ` para prestador ${prestadorId}` : ""}.`);
}

function cmdGerar(args: string[]): void {
  const argComp = args.find((a) => !a.startsWith("-"));
  const competencia = argComp ?? mesCorrente();
  const prestadorId = flag(args, "--prestador") ?? "default";
  ...
  const novas = gerarVendasDoMes(competencia, prestadorId);
  console.log(`Competência ${competencia} [Prestador: ${prestadorId}]: ${novas} venda(s) nova(s) gerada(s).`);
