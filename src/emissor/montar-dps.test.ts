...
test('MEI omite tribFed (E0676)', () => {
  const cfg: ConfigPrestador = { ...CFG, regTrib: { ...CFG.regTrib, opSimpNac: 2 } };
  const { infDps } = montarDps(INPUT, cfg, AGORA);
  assert.ok(!('tribFed' in infDps.valores.trib));
});

test('MEI usa indTotTrib 0 e omite pTotTribSN (E0710)', () => {
  const cfg: ConfigPrestador = { ...CFG, regTrib: { ...CFG.regTrib, opSimpNac: 2 } };
  const { infDps } = montarDps(INPUT, cfg, AGORA);
  assert.equal(infDps.valores.trib.totTrib.indTotTrib, 0);
  assert.ok(!('pTotTribSN' in infDps.valores.trib.totTrib));
});
