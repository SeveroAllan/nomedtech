...
const COMANDOS = {
  ...
  check: {
    ...
    uso: 'nfse check [cTribNac] [--prestador id]',
    desc: 'verifica adesao do municipio do prestador ao Convenio Nacional',
  },
  painel: {
    script: 'server.ts',
    uso: 'nfse painel',
  ...
  contratos: {
    ...
    uso: 'nfse contratos <criar|listar|desativar|gerar> [--prestador id]',
    desc: 'contratos recorrentes: cadastra uma vez, gera as vendas do mes',
  },
  arquivar: {
    script: 'arquivar-nota.ts',
    uso: 'nfse arquivar <id_venda> [--prestador id]',
    desc: 'salva XML e DANFSe da nota em output/<prestador>/notas/<AAAA-MM>/<id_venda>/',
  },
};
